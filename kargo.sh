...
argo_cd_chart_version=9.3.7
argo_rollouts_chart_version=2.40.5
cert_manager_chart_version=v1.19.2
k8s_version=v1.34.3-k3s1
rootdir=$(pwd -P)
cluster_name=kargo-quickstart

k3d cluster create $cluster_name \
  --no-lb \
  --k3s-arg '--disable=traefik@server:0' \
  --image rancher/k3s:$k8s_version \
   --volume /usr/local/share/ca-certificates/wk-certs/wk-zscaler-rootca.crt:/etc/ssl/certs/wk-zscaler-rootca.crt \
  -p '31443-31445:31443-31445@servers:0:direct' \
  -p '32080-32082:32080-32082@servers:0:direct' \
  --wait

# import images that we are not able to pull directly from k3d/k3s due to zscaler
docker pull ecr-public.aws.com/docker/library/redis:8.2.2-alpine
k3d images import  ecr-public.aws.com/docker/library/redis:8.2.2-alpine -c $cluster_name
...
helm install argocd argo-cd \
  ...
  --namespace argocd \
  --create-namespace \
  --values $rootdir/manifests/argocd-values.yaml \
